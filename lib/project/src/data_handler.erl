		...
		{Total_used_data, Data_balance}->
			case Data_Usage of 
				...
				Data_Usage when Data_Usage > Max_data_usage ->
						io:format("this data usage can not be redeemed, because max data usage should not exceed 1MB~n",[]);
				Data_Usage when Data_Usage > Data_balance ->
						io:format("this data usage can not be redeemed, because your data balance is insufficient~n",[]);
